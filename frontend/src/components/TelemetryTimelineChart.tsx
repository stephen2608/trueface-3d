...
export const TelemetryTimelineChart: React.FC<Props> = ({ telemetryHistory }) => {
  ...
  return (
    <div className="bg-gradient-to-b from-zinc-900/90 to-zinc-950/90 rounded-2xl border border-zinc-800/90 p-4 shadow-xl backdrop-blur-xl">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center space-x-2 text-xs font-bold text-white">
          <Activity className="w-3.5 h-3.5 text-blue-400 animate-pulse" />
          <span>Real-Time Motion &amp; Emotion Trends</span>
        </div>
        <div className="flex items-center space-x-4 text-xs">
          <span className="flex items-center space-x-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_#34d399]" />
            <span className="text-zinc-300 font-medium">Smile Warmth</span>
          </span>
          <span className="flex items-center space-x-1.5">
            <span className="w-2 h-2 rounded-full bg-rose-400 shadow-[0_0_6px_#fb7185]" />
            <span className="text-zinc-300 font-medium">Facial Tension</span>
          </span>
          <span className="flex items-center space-x-1.5">
            <span className="w-2 h-2 rounded-full bg-blue-400 shadow-[0_0_6px_#60a5fa]" />
            <span className="text-zinc-300 font-medium">Screen Distance</span>
          </span>
        </div>
      </div>

      <div className="relative w-full h-20 bg-zinc-950/80 rounded-xl overflow-hidden border border-zinc-800/80">
